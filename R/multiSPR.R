...
multiSPR = function(trees1, trees2,
  ...
  if (class(trees1)=="multiPhylo" && class(trees2)=="multiPhylo"){
    ...
    if (method=="meanSPR"){
      ...
      for (i in seq_along(trees1)){
        for (j in seq_along(trees2)){
          ...
          if (normalization==T){spr_distances[i, j] = normalizedSPR(trees1[[i]], trees2[[j]])}
    ...
    # 1.2 If the method is "random"
      ...
      if (normalization==T) {result = normalizedSPR(random_tree1, random_tree2)}
  ...
  # 2. Else, if one or both of the trees are not multiPhylo
    ...
    if (normalization==T){
      result = normalizedSPR(trees1, trees2)
...
'
#TESTE
trees1 = mp_mol_mpts[[24]]$mp_mol_mpts
trees2 = mp_te_mpts[[24]]$mp_te_mpts
trees1
trees2
...
multiSPR(trees1, trees2, method="random", normalization=T)
...
a
'
for (i in 24) {b = normalizedSPR(mp_mol_mpts[[i]]$mp_mol_mpts,
                                mp_te_mpts[[i]]$mp_te_mpts)}
b
